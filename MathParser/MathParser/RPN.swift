...
class RPN
{
    ...
    private func applyOperator(op: Token, first: Double, second: Double?) -> Double
    {
        ...
        case "^": // Potenza.
            if let second = second
            {
                return  pow(second, first) //pow(first, second)
            }
        case "cos": // Coseno.
            return cos(first)
        case "sin": // Seno.
    ...
    func evaluate() -> Double
    {
        ...
        let operands = Stack<Double>()
        
        // Scorro i token:
        for var i = 0; i < output.count; i++
        {
            switch self.RPNTokens[i].type
            {
            case .Number, .Constant, .Unknown:
                operands.push(self.RPNTokens[i].value!)
            case .Operator:
                var operationResult: Double
                
                if self.typeOfOperator(self.RPNTokens[i]) == .Binary
                {
                    operationResult = self.applyOperator(self.RPNTokens[i], first: operands.pop()!, second: operands.pop()!)
                }
                else
                {
                    operationResult = self.applyOperator(self.RPNTokens[i], first: operands.pop()!, second: nil)
                }
            ...
            if output[i].type == .Operator
            {
                ...
                else
                {
                    ...
                    output[i].type = .Number
                    output[i].value = operationResult
